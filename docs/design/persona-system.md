...
### 이정표 A — DialogueContext 정밀화

**문제**: 현재 ContextNarrator의 입력이 거칠어서 "살아있는 듯한 맥락 인식" 경험이 깨짐.
예: 작업을 시작하자마자 "그냥 널 보고 있었어" 같은 장시간 관찰 대사가 나옴.

**현재 DialogueContext 6필드의 한계:**
- `SessionSeconds` — 작업 시간은 있지만, "방금 시작한 직후"와 "10분째 집중 중"을 세분화하지 않음
- `IdleSessionSeconds` — 수집되지만 ContextNarrator에서 미사용
- `Annoyance` — 2단계(0.5 기준)뿐, 점진적 변화 미반영
- 상태 전환 직후의 "전이 맥락"이 없음 (예: "방금 돌아옴" vs "한참 전에 돌아옴")

**개선 방향 (점진적):**
1. **IdleSessionSeconds 활용** — "잠깐 멈춤"(30초) vs "오래 방치"(10분)를 narration에 반영
2. **상태 전이 시간** — 현재 상태에 진입한 지 얼마나 됐는지 (`StateEnteredAt` 또는 `SecondsSinceStateChange`)
3. **이전 상태 기억** — 어디서 왔는지 (Sleeping→WakeUp vs Alert→WakeUp은 다른 맥락)
4. **집중도 추이** — 최근 5분간 상태 변화 빈도 (안정적 작업 vs 왔다갔다)
5. **Annoyance 세분화** — 0.5 경계 대신 3~4단계 (경미/중간/심각/폭발)
6. **요일/날짜 맥락** — 월요일 아침 vs 금요일 저녁의 대사 차이

**검증 기준**: 디버그 모니터에서 narration을 보고 "이 상황에서 이 대사가 나올 법하다"고 느끼는지.

### 이정표 B — 대사 자동 생성의 맥락 인식 추천

**문제**: 현재 AI 대사 생성은 상태(Happy/Alert/...)만 보고 범용적 대사를 추천.
사용자는 내부 파이프라인 구조를 모르므로, "어떤 상황에 대사가 부족한지"를 알 수 없음.

**개선 방향:**
1. **커버리지 분석** — 현재 대사 풀의 SituationDescription을 분석하여, DialogueContext의 어떤 조건 조합에 대사가 없는지 식별
   - 예: "Happy + 세션 3시간 이상"에 맞는 대사가 0개 → "장시간 작업 후 격려 대사가 부족합니다" 안내
   - 예: "WakeUp + 밤늦은 시간"에 맞는 대사가 0개 → 야간 복귀 대사 추천
2. **조건 타겟팅 생성** — AI에게 "이 특정 조건(세션 3시간+, 오후)에 맞는 대사와 상황을 생성해줘"라고 요청
3. **균형 점수 표시** — 상태별 대사 수뿐 아니라, narration 조건 커버리지를 시각화
   - 예: "작업 중 (9개) — 장시간 커버 부족 ⚠️"

**전제 조건**: 이정표 A의 DialogueContext 정밀화가 선행되어야 조건 조합이 의미 있음.
